...
class private_api:
    ...
    def get_rigs(self):
        return self.request('GET', '/main/api/v2/mining/groups/list', 'extendedResponse=True', None)

    def rig_action(self, rigId, action):
        query = "rigId={}&action={}".format(rigId, action)
        actions = {
            "action": action,
            "rigId": rigId
        }
        return self.request('POST', '/main/api/v2/mining/rigs/status2', query, actions)
